...
module.exports = (io) => {
  ...
  router.post("/create-demo", roomCreationLimiter, async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        roomCode: roomCode,
        shareableUrl: `${req.protocol}://${req.get(
          "host"
        )}/room/${roomCode}?demo=true`,
    ...
    try {
      ...
      if (!name || !primaryPassword || !scheduledTime) {
        return res.status(400).json({
          success: false,
          message:
            "Room name, primary password, and scheduled time are required",
        });
      }
  ...
  router.post("/:roomCode/join", authenticateToken, async (req, res) => {
    try {
      ...
      const pendingMember = room.pendingMembers.find(
      ...
      if (
        room.admissionType === "democratic_voting" &&
        username !== room.owner
      ) {
        room.pendingMembers.push({ username });
        await room.save();
    ...
    try {
      ...
      if (room.isDemoRoom) {
        ...
        try {
          ...
          const isMember = room.members.find(
            (member) => member.username === decoded.username
          );
          if (isMember) {
            hasAccess = true;
          }
      ...
      if (messages.length === 0) {
        ...
        messages.forEach((message, index) => {
          ...
          doc.text(
            `[${timestamp}] ${message.username}: ${messageContent}`,
            100,
            yPosition
          );
          yPosition += 20;
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        if (
          !isOwner &&
          (!isMember || room.admissionType !== "democratic_voting")
        ) {
          return res.status(403).json({
            success: false,
            message: "Not authorized",
        ...
        if (room.admissionType === "owner_approval" && isOwner) {
          ...
          const existingVote = pendingMember.votes.find(
          ...
          const admitVotes = pendingMember.votes.filter(
            (vote) => vote.decision === "admit"
          ).length;
          const denyVotes = pendingMember.votes.filter(
            (vote) => vote.decision === "deny"
          ).length;
          ...
          if (totalVotes >= requiredVotes) {
            ...
            return res.json({
              success: true,
              message: `Voting complete. User ${
                finalDecision === "admit" ? "admitted" : "denied"
              }`,
  ...
  // Get room details (with caching for permanent rooms)
  router.get("/room/:id", async (req, res) => {
    const roomId = req.params.id;
    // Only cache permanent rooms
    const roomType = await getRoomType(roomId); // implement this logic
    if (roomType === "permanent") {
      const cachedRoom = await redis.get(`room:${roomId}`);
      if (cachedRoom) return res.json(JSON.parse(cachedRoom));
      const room = await Room.findById(roomId);
      await redis.set(`room:${roomId}`, JSON.stringify(room), "EX", 3600); // 1 hour
      return res.json(room);
    } else {
      // Demo room logic, no cache
      const room = await Room.findById(roomId);
      return res.json(room);
    }
  });
