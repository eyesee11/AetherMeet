...
noteSchema.methods.encryptContent = function (content, password) {
  try {
    ...
    // Encrypt content using createCipheriv
...
noteSchema.methods.decryptContent = function (password) {
  try {
    ...
    // Decrypt content using createDecipheriv
    ...
    throw new Error(
      "Failed to decrypt content - invalid password or corrupted data"
    );
  }
};
...
noteSchema.methods.getDecryptedContent = async function (password) {
  try {
    ...
    if (this.secondaryPassword) {
      const isSecondaryValid = await bcrypt.compare(
        password,
