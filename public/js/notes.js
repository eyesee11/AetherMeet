  ...
  if (urlParams.get("action") === "save-pdf") {
    ...
    if (pdfData && roomCode) {
      ...
      document.getElementById(
        "pdfTitle"
      ).value = `Chat Export - Room ${roomCode}`;
...
async function loadMyNotes() {
  ...
  container.innerHTML =
    '<div class="text-center text-gray-500 py-8 font-mono">Loading notes...</div>';
  ...
  try {
            ...
            (note) => `
                    <div class="bg-white border-3 border-black p-4 hover:bg-gray-50 transition-colors duration-200" style="box-shadow: 4px 4px 0px 0px #000000;">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                <h4 class="font-black text-lg uppercase tracking-tight mb-1">${escapeHtml(
                                  note.title
                                )}</h4>
                            ...
                            <button data-note-code="${
                              note.noteCode
                            }" class="quick-access-btn btn-brutal px-4 py-2 bg-black text-white border-2 border-black font-bold uppercase tracking-wide text-xs">
                                Open
                            </button>
                        </div>
                    </div>
    ...
    container.innerHTML =
      '<div class="text-center text-red-500 py-8 font-mono">Failed to load notes</div>';
  }
}
...
function displayNote(note) {
  ...
  noteInfo.textContent = `Code: ${
    note.noteCode
  } | Type: ${note.noteType.toUpperCase()} | Created: ${new Date(
    note.createdAt
  ).toLocaleString()} | Updated: ${new Date(note.updatedAt).toLocaleString()}`;
